...
//!
//! Reads `testdata/{case}/input.json`, generates HTML matching what Dioxus
//! renders (div elements with CSS properties), captures a headless Chrome
//! screenshot, and saves `rendered_dioxus.png`.
//!
...
fn main() -> Result<()> {
    ...
    for job in &jobs {
        ...
        let out = job.output_dir.join(&job.name).join("rendered_dioxus.png");

        // Write temporary HTML file
        let tmp_html = job.output_dir.join(&job.name).join("_tmp_dioxus.html");
...
// ─── HTML generation ─────────────────────────────────────────────────────────
//
// Generates HTML that matches what Dioxus renders: div elements with inline
// CSS properties. This mirrors the `emit_dioxus` codegen output.
...
<head>
<meta charset="UTF-8">
<style>html,body{margin:0;padding:0;height:100%;width:100%}body{display:flex;flex-direction:column;align-items:flex-start;background:rgba(28,28,43,1)}</style>
</head>
<body>
"#,
    );
...
fn emit_node(
    ...
    if !matches!(
        node.align_content,
        config::AlignContent::Default | config::AlignContent::Stretch
    ) {
        styles.push(format!(
            "align-content: {}",
    ...
    if is_leaf {
        ...
        // Center leaf text like Dioxus would
...
fn css_align_content(a: config::AlignContent) -> &'static str {
    match a {
        ...
        config::AlignContent::Default => "stretch",
    }
}
