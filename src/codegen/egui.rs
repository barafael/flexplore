...
fn emit_egui_node(
    ...
    if is_leaf {
        ...
        *leaf_idx += 1;
        ...
        writeln!(buf, "{pad}    .fill(egui::Color32::from_rgb({r8}, {g8}, {b8}))")?;
        ...
        if has_w || has_h {
            ...
            writeln!(buf, "{pad}        ui.set_min_size(egui::vec2({w_str}, {h_str}));")?;
        }
        ...
        if !is_leaf {
            writeln!(
                buf,
                "{pad}    .fill(egui::Color32::from_rgb(28, 28, 43))"
            )?;
        }
        if let Some(p) = egui_margin(&node.padding) {
            writeln!(buf, "{pad}    .inner_margin({p})")?;
        ...
        if is_root {
            writeln!(
                buf,
                "{pad}        ui.set_min_size(ui.available_size());"
            )?;
        } else {
            // Explicit sizing
            let has_w = !matches!(node.width, ValueConfig::Auto);
            ...
            if parent_stretch {
                if parent_is_row && !has_h {
                    ...
                    writeln!(
                        buf,
                        "{pad}        ui.set_min_width(ui.available_width());"
                    )?;
                }
            }
        }
        ...
        *leaf_idx = acc;
        ...
        for (child, start) in children.iter().zip(starts.iter()) {
            let mut idx = *start;
            emit_egui_node(buf, child, depth + 3, &mut idx, palette, is_row, stretch, false)?;
            writeln!(buf, ";")?;
        }
