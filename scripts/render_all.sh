...
cd "$ROOT_DIR"
FLEXPLORE_UPDATE=1 cargo test snapshot -- --test-threads=1 2>&1 | grep -E "(updated|ok|FAILED)" >&2

echo ""
echo "=== Rendering Bevy screenshots ===" >&2
cargo run --example render_testdata -- "$@" 2>&1 | grep -v "^\[" >&2
...
python "$SCRIPT_DIR/build_overview.py" 2>&1
